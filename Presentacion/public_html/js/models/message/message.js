...
// Model for Message entity
    models.Message = Backbone.Model.extend({
        urlRoot: "http://localhost:10202/Logica/webresources/edu.poli.prap.cv.data.message/",
        ...
        toViewJson: function () {
            ...
            result.displayName = this.get('idMessage');
            return result;
        },
        isNew: function () {
        ...
        sync: function (method, model, options) {
            ...
            if (method == 'create') {
                options.url = 'http://localhost:10202/Logica/webresources/edu.poli.prap.cv.data.message/';
            }
    ...
    // Collection class for Message entities
...
$(function () {
    ...
    var AppRouter = Backbone.Router.extend({
        ...
        list: function () {
            this.collection = new models.MessageCollection();
            var self = this;
            this.collection.fetch({
                success: function () {
                    self.listView = new views.ListView({
                        model: self.collection,
                        // tpl-message-list-itemis template identifier for item
                        templateName: '#tpl-message-list-item'
                    });
        ...
        details: function (id) {
            if (this.collection) {
                this.message = this.collection.get(id);
                if (this.view) {
                    this.view.close();
                }
                var self = this;
                this.view = new views.ModelView({
                    model: this.message,
                    // tpl-message-details is template identifier for chosen model element
                    templateName: '#tpl-message-details',
        ...
        create: function () {
            ...
            var self = this;
            var dataModel = new models.Message();
            // see isNew() method implementation in the model
            dataModel.notSynced = true;
            this.view = new views.ModelView({
                model: dataModel,
                collection: this.collection,
                // tpl-message-details is a template identifier for chosen model element
                templateName: '#tpl-message-details',
