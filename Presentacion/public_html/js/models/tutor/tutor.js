...
// Model for Tutor entity
    ...
    // Collection class for Tutor entities
    models.TutorCollection = Backbone.Collection.extend({
        model: models.Tutor,
...
$(function () {
    ...
    var AppRouter = Backbone.Router.extend({
        ...
        list: function () {
            this.collection = new models.TutorCollection();
            var self = this;
            this.collection.fetch({
                success: function () {
                    self.listView = new views.ListView({
                        model: self.collection,
                        // tpl-tutor-list-itemis template identifier for item
                        templateName: '#tpl-tutor-list-item'
                    });
        ...
        details: function (id) {
            if (this.collection) {
                this.tutor = this.collection.get(id);
                if (this.view) {
                    this.view.close();
                }
                var self = this;
                this.view = new views.ModelView({
                    model: this.tutor,
                    // tpl-tutor-details is template identifier for chosen model element
                    templateName: '#tpl-tutor-details',
        ...
        create: function () {
            ...
            var self = this;
            var dataModel = new models.Tutor();
            // see isNew() method implementation in the model
            dataModel.notSynced = true;
            this.view = new views.ModelView({
                model: dataModel,
                collection: this.collection,
                // tpl-tutor-details is a template identifier for chosen model element
                templateName: '#tpl-tutor-details',
