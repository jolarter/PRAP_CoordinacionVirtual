...
// Model for Subject entity
    ...
    // Collection class for Subject entities
    models.SubjectCollection = Backbone.Collection.extend({
...
$(function () {
    ...
    var AppRouter = Backbone.Router.extend({
        ...
        list: function () {
            this.collection = new models.SubjectCollection();
            var self = this;
            this.collection.fetch({
                success: function () {
                    self.listView = new views.ListView({
                        model: self.collection,
                        // tpl-subject-list-itemis template identifier for item
                        templateName: '#tpl-subject-list-item'
                    });
        ...
        details: function (id) {
            if (this.collection) {
                this.subject = this.collection.get(id);
                if (this.view) {
                    this.view.close();
                }
                var self = this;
                this.view = new views.ModelView({
                    model: this.subject,
                    // tpl-subject-details is template identifier for chosen model element
                    templateName: '#tpl-subject-details',
        ...
        create: function () {
            ...
            var self = this;
            var dataModel = new models.Subject();
            // see isNew() method implementation in the model
            dataModel.notSynced = true;
            this.view = new views.ModelView({
                model: dataModel,
                collection: this.collection,
                // tpl-subject-details is a template identifier for chosen model element
                templateName: '#tpl-subject-details',
