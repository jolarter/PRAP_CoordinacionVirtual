...
// Model for Dictionary entity
    models.Dictionary = Backbone.Model.extend({
        ...
        toViewJson: function () {
            ...
            result.displayName = this.get('idDictionary');
            return result;
        },
        isNew: function () {
    ...
    // Collection class for Dictionary entities
    models.DictionaryCollection = Backbone.Collection.extend({
        model: models.Dictionary,
        url: "http://localhost:10202/Logica/webresources/edu.poli.prap.cv.data.dictionary/",
        sync: function (method, model, options) {
            options || (options = {});
            var errorHandler = {
                error: function (jqXHR, textStatus, errorThrown) {
...
$(function () {
    ...
    var AppRouter = Backbone.Router.extend({
        ...
        list: function () {
            this.collection = new models.DictionaryCollection();
            var self = this;
            this.collection.fetch({
                success: function () {
                    self.listView = new views.ListView({
                        model: self.collection,
                        // tpl-dictionary-list-itemis template identifier for item
                        templateName: '#tpl-dictionary-list-item'
                    });
        ...
        details: function (id) {
            if (this.collection) {
                this.dictionary = this.collection.get(id);
                if (this.view) {
                    this.view.close();
                }
                var self = this;
                this.view = new views.ModelView({
                    ...
                    // tpl-dictionary-details is template identifier for chosen model element
                    templateName: '#tpl-dictionary-details',
        ...
        create: function () {
            ...
            var self = this;
            var dataModel = new models.Dictionary();
            // see isNew() method implementation in the model
            dataModel.notSynced = true;
            this.view = new views.ModelView({
                model: dataModel,
                collection: this.collection,
                // tpl-dictionary-details is a template identifier for chosen model element
                templateName: '#tpl-dictionary-details',
