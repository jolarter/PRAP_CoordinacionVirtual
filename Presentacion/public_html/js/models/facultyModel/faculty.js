...
// Model for Faculty entity
    models.Faculty = Backbone.Model.extend({
        urlRoot: "http://localhost:10202/Logica/webresources/edu.poli.prap.cv.data.faculty/",
        idAttribute: 'idFaculty',
        defaults: {
            name: "",
            description: ""
        },
        ...
        sync: function (method, model, options) {
            ...
            if (method == 'create') {
                options.url = 'http://localhost:10202/Logica/webresources/edu.poli.prap.cv.data.faculty/';
            }
    ...
    // Collection class for Faculty entities
    models.FacultyCollection = Backbone.Collection.extend({
        model: models.Faculty,
        url: "http://localhost:10202/Logica/webresources/edu.poli.prap.cv.data.faculty/",
        sync: function (method, model, options) {
            options || (options = {});
            var errorHandler = {
                error: function (jqXHR, textStatus, errorThrown) {
...
$(function () {
    ...
    var AppRouter = Backbone.Router.extend({
        ...
        list: function () {
            this.collection = new models.FacultyCollection();
            var self = this;
            this.collection.fetch({
                success: function () {
                    self.listView = new views.ListView({
                        model: self.collection,
                        // tpl-faculty-list-itemis template identifier for item
                        templateName: '#tpl-faculty-list-item'
                    });
        ...
        details: function (id) {
            if (this.collection) {
                this.faculty = this.collection.get(id);
                if (this.view) {
                    this.view.close();
                }
                var self = this;
                this.view = new views.ModelView({
                    model: this.faculty,
                    // tpl-faculty-details is template identifier for chosen model element
                    templateName: '#tpl-faculty-details',
        ...
        create: function () {
            ...
            var self = this;
            var dataModel = new models.Faculty();
            // see isNew() method implementation in the model
            dataModel.notSynced = true;
            this.view = new views.ModelView({
                model: dataModel,
                collection: this.collection,
                // tpl-faculty-details is a template identifier for chosen model element
                templateName: '#tpl-faculty-details',
