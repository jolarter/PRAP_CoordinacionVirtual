...
// Model for Module entity
    models.Module = Backbone.Model.extend({
        urlRoot: "http://localhost:8080/Logica/webresources/edu.poli.prap.cv.data.module/",
        ...
        sync: function (method, model, options) {
            ...
            if (method == 'create') {
                options.url = 'http://localhost:8080/Logica/webresources/edu.poli.prap.cv.data.module/';
            }
    ...
    // Collection class for Module entities
    models.ModuleCollection = Backbone.Collection.extend({
        model: models.Module,
...
$(function () {
    ...
    var AppRouter = Backbone.Router.extend({
        ...
        list: function () {
            this.collection = new models.ModuleCollection();
            var self = this;
            this.collection.fetch({
                success: function () {
                    self.listView = new views.ListView({
                        model: self.collection,
                        // tpl-module-list-itemis template identifier for item
                        templateName: '#tpl-module-list-item'
                    });
        ...
        details: function (id) {
            if (this.collection) {
                this.module = this.collection.get(id);
                if (this.view) {
                    this.view.close();
                }
                var self = this;
                this.view = new views.ModelView({
                    model: this.module,
                    // tpl-module-details is template identifier for chosen model element
                    templateName: '#tpl-module-details',
        ...
        create: function () {
            ...
            var self = this;
            var dataModel = new models.Module();
            // see isNew() method implementation in the model
            dataModel.notSynced = true;
            this.view = new views.ModelView({
                model: dataModel,
                collection: this.collection,
                // tpl-module-details is a template identifier for chosen model element
                templateName: '#tpl-module-details',
