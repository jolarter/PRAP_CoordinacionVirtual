...
// Model for Cohort entity
    models.Cohort = Backbone.Model.extend({
    ...
    // Collection class for Cohort entities
    models.CohortCollection = Backbone.Collection.extend({
        model: models.Cohort,
...
$(function () {
    ...
    var AppRouter = Backbone.Router.extend({
        ...
        list: function () {
            this.collection = new models.CohortCollection();
            var self = this;
            this.collection.fetch({
                success: function () {
                    self.listView = new views.ListView({
                        model: self.collection,
                        // tpl-cohort-list-itemis template identifier for item
                        templateName: '#tpl-cohort-list-item'
                    });
        ...
        details: function (id) {
            if (this.collection) {
                this.cohort = this.collection.get(id);
                if (this.view) {
                    this.view.close();
                }
                var self = this;
                this.view = new views.ModelView({
                    model: this.cohort,
                    // tpl-cohort-details is template identifier for chosen model element
                    templateName: '#tpl-cohort-details',
        ...
        create: function () {
            ...
            var self = this;
            var dataModel = new models.Cohort();
            // see isNew() method implementation in the model
            dataModel.notSynced = true;
            this.view = new views.ModelView({
                model: dataModel,
                collection: this.collection,
                // tpl-cohort-details is a template identifier for chosen model element
                templateName: '#tpl-cohort-details',
