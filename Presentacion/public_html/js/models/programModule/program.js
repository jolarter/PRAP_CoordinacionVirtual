...
// Model for Program entity
    models.Program = Backbone.Model.extend({
        urlRoot: "http://localhost:10202/Logica/webresources/edu.poli.prap.cv.data.program/",
        ...
        sync: function (method, model, options) {
            ...
            if (method == 'create') {
                options.url = 'http://localhost:10202/Logica/webresources/edu.poli.prap.cv.data.program/';
            }
    ...
    // Collection class for Program entities
    models.ProgramCollection = Backbone.Collection.extend({
        model: models.Program,
        url: "http://localhost:10202/Logica/webresources/edu.poli.prap.cv.data.program/",
        sync: function (method, model, options) {
            options || (options = {});
            var errorHandler = {
                error: function (jqXHR, textStatus, errorThrown) {
...
$(function () {
    ...
    var AppRouter = Backbone.Router.extend({
        ...
        list: function () {
            this.collection = new models.ProgramCollection();
            var self = this;
            this.collection.fetch({
                success: function () {
                    self.listView = new views.ListView({
                        model: self.collection,
                        // tpl-program-list-itemis template identifier for item
                        templateName: '#tpl-program-list-item'
                    });
        ...
        details: function (id) {
            if (this.collection) {
                this.program = this.collection.get(id);
                if (this.view) {
                    this.view.close();
                }
                var self = this;
                this.view = new views.ModelView({
                    model: this.program,
                    // tpl-program-details is template identifier for chosen model element
                    templateName: '#tpl-program-details',
        ...
        create: function () {
            ...
            var self = this;
            var dataModel = new models.Program();
            // see isNew() method implementation in the model
            dataModel.notSynced = true;
            this.view = new views.ModelView({
                model: dataModel,
                collection: this.collection,
                // tpl-program-details is a template identifier for chosen model element
                templateName: '#tpl-program-details',
